...
本号内容按**商业宣传**口径受《广告法》约束，违规会导致文章下架与账号处罚。
以下三条为**硬约束**，优先级高于本文档其余所有写作建议。

**法律依据**：《广告法》第九条第(二)项——不得使用或者变相使用**国家机关、国家机关工作人员的名义或者形象**；
以及未经许可从事**证券投资咨询**的禁止性规定。

### 一、不得出现政要的姓名或职务指代

- 政府首脑、国家元首、部长级官员的**姓名一律不出现**（含中英文、简体繁体、常见变体）；
- **职务指代同禁**——"某国总统""某国国家主席"属条文里的"**变相**使用"；
- **不要描写外交/政治事件本身**（峰会、会晤、国事访问）。政策变量只写**与产业链相关的动作**
  （如"关键矿物出口管制收紧"）——那才是论证需要的部分，会晤叙事对产业论证毫无必要。

### 二、不得对具体证券下判断或给操作建议

**判断只允许针对环节/材料/产能/时间窗；公司名只能作事实主体。**

| 允许 ✅ | 禁止 ❌ |
|---|---|
| 谁发布了什么、涨跌多少、收盘价、市值 | 价格区间（"观察区间 13.5–15 美元"） |
| PE/PS/市值/涨跌幅等中性数据（须标口径与日期） | 我方情景测算、以及"较现价 +X%"这类换算 |
| 产能、缺口、扩产周期、市占率等产业事实 | 操作动词：买入/加仓/不追高/维持观察/止损 |
| 瓶颈分级（S/A/B 级——对**环节**的判断） | `安全边际`、个股 `★` 评级、板块级提示 |
| "某机构目标价区间"（引用，**不换算成收益**） | `纯正标的`/`最纯正`/`首选`/`弹性最大`/`最值得跟踪` |

**判据一**：这句话是在说"这家公司**是什么**"（事实，可留），还是在说"这家公司**是最好的**"（选择，删）。
**判据二**：估值数据后面**不得跟结论**——"PE 304 倍"是事实；"PE 304 倍，情绪远超盈利支撑"是对证券的分析意见。
**判据三**：`唯一`/`龙头` 等词**按组合判**——"唯一供应商"是事实，"唯一…的标的"是选择。
...
**免责不免责**——它只声明内容性质，不能使越线内容变合法；正文成色决定它的价值。

### 机检与人工的分工（不要指望机器兜住全部）

发布管线在调微信接口前跑 `tools/compliance_lint.py`：**红线一与红线二的词汇级部分硬阻断**
（管线退出码 **7**，**无放行开关**）；国家机关名称、工作人员泛称、时政叙事词**只告警**。
词表见 `tools/compliance_blocklist.json`（数据唯一来源，本文件是规则唯一来源）。

**机器抓不到的部分**：判断对象是证券还是环节属**语义级**，闸门判不了——
`PE 304 倍，情绪远超盈利支撑` 与 `InP 缺口 70%，扩产周期 18 个月` 是同一个句式。
这一层只能靠上面三条判据**人工自查**，没有机制兜底。
...
| 类型 | 路径 | 命名格式 |
|------|------|---------|
| 公司分析类 | `reports/公司研究/{公司名}/` | `{公司名}-公众号-{YYYYMMDD}.md` |
| 行业分析类 | `reports/行业分析/` | `公众号-{行业关键词}-{YYYYMMDD}.md` |
| 投资理念类 | `reports/投资理念/` | `公众号-{主题关键词}-{YYYYMMDD}.md` |
| 阅读扩展类 | `reports/阅读扩展/` | `公众号-{主题关键词}-{YYYYMMDD}.md` |
