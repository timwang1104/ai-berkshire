...
检查是否已存在该公司的投资论文文件（`reports/公司研究/{公司名}/{公司名}-thesis.md`）：
- 如果不存在 → 进入**建立论文**模式
- 如果存在 → 进入**追踪检查**模式
- 如果找不到但用户表示已有 → 询问文件路径
...
将投资论文写入 `reports/公司研究/{公司名}/{公司名}-thesis.md`，包含：
- 建立日期
- 买入价格和仓位
- 核心论文（5句话）
- 核心假设清单
...
读取 `reports/公司研究/{公司名}/{公司名}-thesis.md`，加载：
- 核心论文
- 核心假设清单
- 红线清单
- 上次检查记录
...
将本次检查记录追加到 `reports/公司研究/{公司名}/{公司名}-thesis.md` 的追踪记录表中：
